...
return 1; 
	...
	public static int fibonaciIterativno(int n){ 
		...
		fibonaci[0] = 1; //ako je n bar 1, inicijalizuje se i prvi element na 1 
		if(n >= 1) 
			fibonaci[1] = 1; 
		//za svako i, 2 <= i <= n, primenjuje se rekurentna formula 
		for(int i = 2; i < n+1; i++) 
			fibonaci[i] = fibonaci[i-1] + fibonaci[i-2]; 
		//n-ti član Fibonačijevog niza je //element fibonaci[n] dobijenog niza 
		return fibonaci[n]; } 
	public static void main(String[] args){ 
		Scanner ulaz = new Scanner(System.in); // Broj za koji se računa n-ti član Fibonačijevog niza 
		int n; 
		System.out.print("Unesite ceo nenegativan broj " + " - negativan za kraj: "); while((n = ulaz.nextInt()) >= 0){ 
			// Rekurzivna varijanta 
			long vremePocetak = System.nanoTime(); 
			int fibonaciR = Primer11b.fibonaciRekurzivno(n); 
			long vremeKraj = System.nanoTime(); System.out.println("Rekurzivna varijanta:"); 
			System.out.println("Fibonači(" + n + ") = " + fibonaciR); 
			System.out.println("Vreme: " + (vremeKraj - vremePocetak) + "ns");
			vremePocetak = System.nanoTime(); int fibonaciI = Primer11b.fibonaciIterativno(n); 
