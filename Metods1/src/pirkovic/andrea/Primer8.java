...
public class Primer8 { 
	static double suma(double[] a, double[] b, int k, int n, int m) { 
		double s = 0; 
		for (int i = 1; i <= k; i ++) s += Math.pow(a[i], n) * Math.pow(b[i], m); 
	...
	public static void main(String[] args) throws Exception { 
		...
		for (int i = 1; i <= n; i++) { 
			System.out.print("x[" + i + "] = "); x[i] = Double.parseDouble(ulaz.readLine()); } 
		System.out.println("Unesite elemente niza y: "); for (int i = 1; i <= n; i++) { System.out.print("y[" + i + "] = "); y[i] = Double.parseDouble(ulaz.readLine()); } 
